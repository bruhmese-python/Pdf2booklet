...
try:
    ...
    config.read("init.ini", encoding='utf-8-sig')
    POPPLER_PATH = str(config['DEFAULT']['poppler'])
except Exception as e:
    messagebox.showerror("Error", "Corrupt config file")
    exit()
...
def split_images(PDF_FILE, DPI):
    ...
    PAGES = convert_from_path(
        PDF_FILE, DPI, poppler_path=POPPLER_PATH)
    chdir(dir)
